...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty() && curr.getWord()!= word2) {
			curr = queue.remove(0);
			distanceOne = nw.distanceOne(curr.getWord(), true);
			for(String s: distanceOne) {
				if(!visited.contains(s)) {
					WPTreeNode aNode = new WPTreeNode(s, curr);
					visited.add(s);
					queue.add(queue.size(), aNode);
					if(s.equals(word2)) {
						return aNode.buildPathToRoot();
					}
				}
			}
		}
