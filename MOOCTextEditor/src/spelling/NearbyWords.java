...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index <= s.length(); index++) {
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				boolean isInDict = dict.isWord(sb.toString());
				boolean testWords = (!wordsOnly||dict.isWord(sb.toString()));
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size() < numSuggestions && numCompare <= THRESHOLD) {
			String aWord = queue.remove(0);
			List<String> aList = distanceOne(aWord, true);
			for(String s:aList) {
   ...
   public static void main(String[] args) {
	   ...
	   NearbyWords w1 = new NearbyWords(d);
 	   List<String> l = w.distanceOne(word, true);
	   List<String> wordsOnly = w1.distanceOne(word1, false);
